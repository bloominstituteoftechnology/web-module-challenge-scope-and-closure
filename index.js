...
/*Task 1: counterMaker()
  ...
  1. What is the difference between counter1 and counter2?
    They work the same way except once uses a global variable and one uses a local variable.
...
Use the getInningScore() function below to do the following:
  ...
  function getInningScore(inningNumber) {
...
function InningScore(innings, number){
  const score = [];
  let home = 0;
  let away = 0;
  
  for(let i = 0; i < 9; i++){
    const newScore = number(innings);
    home = home + newScore.Home,
    away = away + newScore.Away
    score.push(`Inning ${i + 1} Away: ${newScore.Away} - Home: ${newScore.Home}`);
    }
  
  return score;
}

console.log(InningScore(inning, getInningScore));
